...
static int tbl_ent_insert(HashTblEnt* ent, int64_t key, int isIntKey, const void* value, uint32_t hash, uint32_t elemSize)
{
    ...
    {
        Buffer* bkey = (Buffer*)key;

        ent->keyStr = bkey;
        ent_set_str_key(ent);
        buf_grab(bkey); /* The HashTbl always grabs an inserted string key */
    }
        ...
        for (;;)
        {
            if (ent->hash == hash && ent_is_int_key(ent) == (uint32_t)isIntKey)
            {
                ...
                else
                {
                    ...
                    if (buf_length(ent->keyStr) == len && strncmp(buf_str(ent->keyStr), buf_str(bkey), len) == 0)
                    {
                        if (updateInPlace)
                        {
                            /* If this is a different Buffer object, we drop the old one and grab the new one */
                            buf_drop(ent->keyStr);
                            buf_grab(bkey);
                            goto update;
                        }
...
{
    return tbl_set_impl(tbl, (int64_t)key, false, value, hash_cstr(buf_str(key), buf_length(key)), false);
}
...
{
    return tbl_set_impl(tbl, (int64_t)key, false, value, hash_cstr(buf_str(key), buf_length(key)), true);
}
...
static void* tbl_get_impl(HashTbl* tbl, int64_t key, uint32_t len, int isIntKey, uint32_t hash)
{
    ...
    for (;;)
    {
        if (ent->hash == hash && ent_is_int_key(ent) == (uint32_t)isIntKey)
        {
            ...
            {
                if (buf_length(ent->keyStr) == len && strncmp(buf_str(ent->keyStr), (const char*)key, len) == 0)
                    return ent->data;
            }
        }
...
{
    uint32_t hash = hash_cstr(key, len);
    uint32_t capMinusOne = tbl->capacity - 1;
    uint32_t size = tbl->entSize;
    uint32_t pos = hash & capMinusOne;
    byte* data = tbl->data;
    ...
    for (;;)
    {
        ...
        {
            if (buf_length(ent->keyStr) == len && strncmp(buf_str(ent->keyStr), key, len) == 0)
                return ent->keyStr;
        }
